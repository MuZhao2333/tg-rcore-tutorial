...
void I_SetPalette (byte* palette)
{
    ...
    // 同步调色板到 doomgeneric_tg.c 的 DG_Palette（供 DG_DrawFrame 使用）
    extern unsigned char DG_Palette[256 * 3];
    palette -= 768;  // 回退到调色板开头
    for (i = 0; i < 256; ++i) {
        DG_Palette[i * 3 + 0] = palette[i * 3 + 0];
        DG_Palette[i * 3 + 1] = palette[i * 3 + 1];
        DG_Palette[i * 3 + 2] = palette[i * 3 + 2];
    }
