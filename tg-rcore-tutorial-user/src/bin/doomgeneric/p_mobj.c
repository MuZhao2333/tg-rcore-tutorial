  ...
  statenum_t	state )
{
	...
	// Guard against NULL action (prevents LoadPageFault if state data is corrupted)
	if (st && st->action.acp1)
	    st->action.acp1(mobj);	
...
{ 	
	...
	if ( player&&player->mo&&(unsigned)((player->mo->state - states)- S_PLAY_RUN1) < 4)
